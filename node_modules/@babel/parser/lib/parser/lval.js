    ...
    switch (expr.type) {
      case 'Identifier':
        if (
          this.state.strict &&
          (strictModeChanged
            ? (0, _identifier.isStrictBindReservedWord)(expr.name, this.inModule)
            : (0, _identifier.isStrictBindOnlyReservedWord)(expr.name))
        ) {
      ...
      default: {
        const message =
          (bindingType === _scopeflags.BIND_NONE ? 'Invalid' : 'Binding invalid') +
          ' left-hand side' +
