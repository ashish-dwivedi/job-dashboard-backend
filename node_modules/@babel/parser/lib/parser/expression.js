...
class ExpressionParser extends _lval.default {
  ...
  parseExprOp(left, leftStartPos, leftStartLoc, minPrec, noIn) {
    ...
    if (prec != null && (!noIn || !this.match(_types.types._in))) {
      if (prec > minPrec) {
        ...
        if (
          (coalesce && (nextOp === _types.types.logicalOR || nextOp === _types.types.logicalAND)) ||
          (logical && nextOp === _types.types.nullishCoalescing)
        ) {
    ...
    while (!this.eat(close)) {
      ...
      elts.push(
        this.parseExprListItem(
          false,
          possibleAsyncArrow ? new _util.ExpressionErrors() : undefined,
          possibleAsyncArrow
            ? {
                start: 0,
              }
  ...
  isAsyncProp(prop) {
    return (
      ...
      (this.match(_types.types.name) ||
        this.match(_types.types.num) ||
        this.match(_types.types.string) ||
        this.match(_types.types.bracketL) ||
        this.state.type.keyword ||
        this.match(_types.types.star)) &&
  ...
  parsePropertyName(prop, isPrivateNameAllowed) {
    if (this.eat(_types.types.bracketL)) {
      ...
      prop.key =
        this.match(_types.types.num) || this.match(_types.types.string) || this.match(_types.types.bigint)
          ? this.parseExprAtom()
  ...
  parseMethod(node, isGenerator, isAsync, isConstructor, allowDirectSuper, type, inClassScope = false) {
    ...
    this.scope.enter(
      (0, _scopeflags.functionFlags)(isAsync, node.generator) |
        _scopeflags.SCOPE_SUPER |
        (inClassScope ? _scopeflags.SCOPE_CLASS : 0) |
        (allowDirectSuper ? _scopeflags.SCOPE_DIRECT_SUPER : 0)
    );
  ...
  checkReservedWord(word, startLoc, checkKeywords, isBinding) {
    ...
    const reservedTest = !this.state.strict
      ? _identifier.isReservedWord
      : isBinding
      ? _identifier.isStrictBindReservedWord
      : _identifier.isStrictReservedWord;
  ...
  parseAwait() {
    ...
    if (!this.scope.inFunction && !this.options.allowAwaitOutsideFunction) {
      if (
        this.hasPrecedingLineBreak() ||
        this.match(_types.types.plusMin) ||
        this.match(_types.types.parenL) ||
        this.match(_types.types.bracketL) ||
        this.match(_types.types.backQuote) ||
        this.match(_types.types.regexp) ||
        this.match(_types.types.slash) ||
