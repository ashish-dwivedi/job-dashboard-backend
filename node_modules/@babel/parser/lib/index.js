...
types._function.updateContext = types._class.updateContext = function(prevType) {
  if (
    ...
    !(prevType === types._return && lineBreak.test(this.input.slice(this.state.lastTokEnd, this.state.start))) &&
        ...
        case types.parenL:
          ...
          if (isGroupedType) {
            ...
            if (
              ...
              !(this.match(types.comma) || (this.match(types.parenR) && this.lookahead().type === types.arrow))
            ) {
              this.expect(types.parenR);
              return type;
            } else {
      ...
      if ((jsx && jsx.error) || this.isRelational('<')) {
        ...
        const arrow = this.tryParse(() => {
          ...
          const arrowExpression = this.forwardNoArrowParamsConversionAt(typeParameters, () =>
            super.parseMaybeAssign(noIn, refExpressionErrors, afterLeftParse, refNeedsArrowPos)
          );
...
function validatePlugins(plugins) {
  ...
  if (hasPlugin(plugins, 'pipelineOperator') && !PIPELINE_PROPOSALS.includes(getPluginOption(plugins, 'pipelineOperator', 'proposal'))) {
    throw new Error(
      "'pipelineOperator' requires 'proposal' option whose value should be one of: " + PIPELINE_PROPOSALS.map(p => `'${p}'`).join(', ')
    );
  }
}
const mixinPlugins = {
...
class CommentsParser extends BaseParser {
  ...
  processComment(node) {
    ...
    if (firstChild) {
      ...
      ((this.state.commentPreviousNode.type === 'ImportSpecifier' && node.type !== 'ImportSpecifier') ||
        (this.state.commentPreviousNode.type === 'ExportSpecifier' && node.type !== 'ExportSpecifier'))
    ) {
  ...
  readToken_plus_min(code) {
    ...
    if (next === code) {
      if (
        ...
        (this.state.lastTokEnd === 0 || lineBreak.test(this.input.slice(this.state.lastTokEnd, this.state.pos)))
      ) {
...
class UtilParser extends Tokenizer {
  ...
  isUnparsedContextual(nameStart, name) {
    ...
    return (
      this.input.slice(nameStart, nameEnd) === name && (nameEnd === this.input.length || !isIdentifierChar(this.input.charCodeAt(nameEnd)))
    );
  }
  ...
  checkLVal(expr, bindingType = BIND_NONE, checkClashes, contextDescription, disallowLetBinding, strictModeChanged = false) {
    switch (expr.type) {
      case 'Identifier':
        if (
          this.state.strict &&
          (strictModeChanged ? isStrictBindReservedWord(expr.name, this.inModule) : isStrictBindOnlyReservedWord(expr.name))
        ) {
...
class ExpressionParser extends LValParser {
  ...
  parsePropertyName(prop, isPrivateNameAllowed) {
    if (this.eat(types.bracketL)) {
      ...
      prop.key =
        this.match(types.num) || this.match(types.string) || this.match(types.bigint)
          ? this.parseExprAtom()
  ...
  parseMethod(node, isGenerator, isAsync, isConstructor, allowDirectSuper, type, inClassScope = false) {
    ...
    this.scope.enter(
      functionFlags(isAsync, node.generator) | SCOPE_SUPER | (inClassScope ? SCOPE_CLASS : 0) | (allowDirectSuper ? SCOPE_DIRECT_SUPER : 0)
    );
  ...
  parseIdentifierName(pos, liberal) {
    ...
    if (this.match(types.name)) {
      ...
      if (
        ...
        (this.state.lastTokEnd !== this.state.lastTokStart + 1 || this.input.charCodeAt(this.state.lastTokStart) !== 46)
      ) {
        this.state.context.pop();
      }
    } else {
...
class StatementParser extends ExpressionParser {
  ...
  registerFunctionStatementId(node) {
    ...
    this.scope.declareName(
      node.id.name,
      this.state.strict || node.generator || node.async ? (this.scope.treatFunctionsAsVar ? BIND_VAR : BIND_LEXICAL) : BIND_FUNCTION,
      node.id.start
    );
  }
